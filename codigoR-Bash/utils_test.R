load_test <- function(){
  if(!file.exists('../Data/walmart_test.rds')){
    walmart_test <- read_csv('../Data/test.csv',
                             col_names=TRUE)#,
    #                              na = "?")
    saveRDS(walmart_test, "../Data/walmart_test.rds")
    print('walmart_test.rds se guardó')
  }
  else{
    warning('walmart_test.rds ya existe')
    walmart_test <- readRDS("../Data/walmart_test.rds")
  }
...
walmart_clean_data_test <- function(x){
  str_replace_all(x,c(','='',' - '='_' , '  '='_','-' = '_', '__'='_', '___'='_',' '='_', '/'='_'))#, '\\?'='(other)'))
}

save.rds.wal.test <- function(num){
  saveRDS(walmart_test, paste("../Data/walmart_test_",num,".rds",sep=""))
  print(paste("walmart_test_",num,".rds"," se guardó",sep=""))
}

add.variables.test<- function(df){
  source("groups2.R")
  df %>%
...
generate_groups_test<-function(df){
...
change_description_test <- function(df){
