...
```{r}
# Eliminación de los NAs de la BD.
Datos_sin_NAs <- train[!is.na(train$FinelineNumber),]
```

```{r}
md.pattern(Datos_sin_NAs)
```
...
```{r}
ggplot(data = train) +
  geom_point(mapping = aes(x = ScanCount, y = FinelineNumber, color = Weekday))
```
...
```{r, eval=FALSE}
c1 = roc(Datos_sin_NAs$TripType ~ Datos_sin_NAs$VisitNumber)
ci.auc(c1)
c1$auc # Area de la curva de ROC  0.5394
plot(c1)

c2 = roc(Datos_sin_NAs$TripType ~ Datos_sin_NAs$ScanCount)
ci.auc(c2)
c2$auc   # 0.5211
plot(c2)

c3 = roc(Datos_sin_NAs$TripType ~ Datos_sin_NAs$FinelineNumber)
ci.auc(c3)
c3$auc    # 0.8618
plot(c3)

# Con dato numérico en lugar de categórico
# c4 = roc(Datos_sin_NAs$TripType ~ Datos_sin_NAs$DepartmentDescription)
# ci.auc(c4)
# c4$auc   # 0.8301
# plot(c4)

# c5 = roc(train$TripType ~ train$Weekday)
# ci.auc(c5)
# c5$auc    # 0.5201
# plot(c5)  
```
